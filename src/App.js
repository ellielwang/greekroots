    ...
    <div style={{ width: "100vw", height: "100vh", background: "#06060f", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'DM Sans',sans-serif" }}>
      ...
      <div style={{ width: 360, background: "#0c0c1c", border: "1px solid #1a1a2e", borderRadius: 16, padding: 36 }}>
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 26, color: "#f5f5f5" }}>
            ΑΣΡ <span style={{ fontStyle: "italic", fontWeight: 400, fontSize: 20, color: "#c0392b" }}>Family Tree</span>
          </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <div>
            <div style={{ fontSize: 11, color: "#444", marginBottom: 4 }}>Email</div>
            <input
              type="email" placeholder="you@uga.edu" value={email}
              onChange={e => setEmail(e.target.value)}
              onKeyDown={e => e.key === "Enter" && handleLogin()}
              style={{ width: "100%", background: "#101020", border: "1px solid #22223a", borderRadius: 8, color: "#e8e8f0", padding: "9px 12px", fontFamily: "'DM Sans',sans-serif", fontSize: 13, outline: "none" }}
            />
          </div>
          <div>
            <div style={{ fontSize: 11, color: "#444", marginBottom: 4 }}>Password</div>
            <input
          ...
          <button
            ...
            style={{ marginTop: 4, background: "#c0392b", color: "#fff", border: "none", borderRadius: 8, padding: "10px 0", fontSize: 13, fontWeight: 600, cursor: loading ? "wait" : "pointer", fontFamily: "'DM Sans',sans-serif", opacity: loading ? 0.7 : 1 }}>
            {loading ? "Signing in…" : "Sign In"}
          </button>
        </div>

        <div style={{ marginTop: 20, fontSize: 11, color: "#2a2a3a", textAlign: "center" }}>
          Access is restricted to chapter leadership.<br />Contact your NMD or President to request access.
        </div>
      </div>
    </div>
  );
...
function AdminDashboard({ currentUser, onClose }) {
  ...
  const createUser = async () => {
    ...
    if (data.user) {
      await supabase.from("user_roles").upsert({ id: data.user.id, email: inviteEmail.trim(), role: inviteRole });
      setInviteSuccess(`Account created for ${inviteEmail.trim()}!`);
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#06060fcc", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'DM Sans',sans-serif" }}
      onClick={onClose}>
      <div style={{ width: 600, maxHeight: "85vh", background: "#0c0c1c", border: "1px solid #1a1a2e", borderRadius: 16, overflow: "hidden", display: "flex", flexDirection: "column" }}
        ...
        <div style={{ padding: "18px 24px", borderBottom: "1px solid #1a1a2e", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <div style={{ fontSize: 16, fontWeight: 600, color: "#f0f0f0" }}>User Management</div>
            <div style={{ fontSize: 11, color: "#444", marginTop: 2 }}>Admin only · {users.length} accounts</div>
          </div>
        ...
        <div style={{ overflowY: "auto", flex: 1 }}>
          ...
          <div style={{ padding: "18px 24px", borderBottom: "1px solid #0f0f1f" }}>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              ...
              <select style={{ ...inp, flex: 1, minWidth: 100 }} value={inviteRole} onChange={e => setInviteRole(e.target.value)}>
                <option value="member">Member</option>
                <option value="admin">Admin</option>
              </select>
              <button onClick={createUser} disabled={inviting}
                style={{ background: "#c0392b", color: "#fff", border: "none", borderRadius: 7, padding: "8px 14px", fontSize: 13, fontWeight: 500, cursor: inviting ? "wait" : "pointer", whiteSpace: "nowrap", opacity: inviting ? 0.7 : 1 }}>
          ...
          <div style={{ padding: "18px 24px" }}>
            ...
            {loading ? (
              <div style={{ color: "#333", fontSize: 13, padding: "20px 0" }}>Loading…</div>
            ) : users.length === 0 ? (
              <div style={{ color: "#333", fontSize: 13, padding: "20px 0" }}>No accounts yet.</div>
            ) : (
...
export default function App() {
  ...
  const handleSignOut = async () => {
    ...
    setSession(null); setUserRole(null); setShowDashboard(false);
  };
  ...
  const stats = { total: members.length, classes: CLASS_ORDER.length };
  ...
  if (authLoading) return (
    <div style={{ width: "100vw", height: "100vh", background: "#06060f", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ color: "#222", fontFamily: "'DM Sans',sans-serif", fontSize: 13 }}>Loading…</div>
    </div>
  );
    ...
    <div style={{ width: "100vw", height: "100vh", background: "#06060f", fontFamily: "'DM Sans',sans-serif", overflow: "hidden", position: "relative", cursor: isPanning ? "grabbing" : "grab" }}
      ...
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, zIndex: 10, padding: "14px 20px", display: "flex", alignItems: "center", gap: 14, background: "linear-gradient(180deg,#06060f 60%,transparent)", pointerEvents: "none" }}>
        ...
        <div style={{ pointerEvents: "all", display: "flex", alignItems: "center", gap: 8 }}>
          {isAdmin && (
            <>
              <button className="btn bp" onClick={() => { setPanel("add"); setSelected(null); }}>+ Add</button>
              <button className="btn bg" onClick={() => setShowDashboard(true)} title="User Management"
                style={{ padding: "8px 10px", fontSize: 15 }}>⚙</button>
            </>
          )}
          <div style={{ display: "flex", alignItems: "center", gap: 8, background: "#101020", border: "1px solid #1a1a2e", borderRadius: 8, padding: "5px 10px" }}>
            ...
            <span style={{ fontSize: 11, color: "#555", maxWidth: 130, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{session.user.email}</span>
      ...
      <div style={{ position: "absolute", top: 68, left: 20, zIndex: 9, display: "flex", gap: 6, flexWrap: "wrap", pointerEvents: "all" }}>
        <button onClick={() => setDynastyFilter(null)} style={{ padding: "4px 10px", borderRadius: 20, border: "1px solid", fontSize: 11, fontWeight: 500, cursor: "pointer", borderColor: !dynastyFilter ? "white" : "#2a2a3a", background: !dynastyFilter ? "#c0392b" : "transparent", color: !dynastyFilter ? "white" : "#666", transition: "all .2s" }}>
          All Lines
        </button>
        {DYNASTIES.map(d => (
          <button key={d} onClick={() => setDynastyFilter(dynastyFilter === d ? null : d)} style={{ padding: "4px 10px", borderRadius: 20, border: `1px solid ${DYNASTY_COLORS[d]}`, fontSize: 11, fontWeight: 500, cursor: "pointer", background: dynastyFilter === d ? DYNASTY_COLORS[d] + "33" : "transparent", color: dynastyFilter === d ? DYNASTY_COLORS[d] : "#666", transition: "all .2s" }}>
            {d}
          </button>
        ))}
      </div>
      ...
      <div ref={svgRef} style={{ width: "100%", height: "100%", overflow: "hidden" }}>
        <svg style={{ transform: `translate(${pan.x}px,${pan.y}px) scale(${zoom})`, transformOrigin: "0 0", willChange: "transform" }} width={svgW} height={svgH}>
          ...
          {edges.map(({ id, from, to, inFocus, member }) => {
            ...
            return (
              <path key={id} d={`M${fx} ${fy} C${fx} ${my},${tx} ${my},${tx} ${ty}`} fill="none"
      ...
      {showDashboard && isAdmin && (
        <AdminDashboard currentUser={session.user} onClose={() => setShowDashboard(false)} />
      )}
    </div>
  );
}
